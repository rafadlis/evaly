...
const buttonVariants = cva(
  "inline-flex items-center justify-center border border-transparent whitespace-nowrap text-sm font-medium disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 [&_svg]:shrink-0 transition-all cursor-pointer active:opacity-90",
  {
    variants: {
      variant: {
        default:
      ...
      size: {
        default: "h-9 px-5 has-[>svg]:pl-3 gap-2 ",
        sm: "h-8 px-4 has-[>svg]:pl-2.5 gap-1.5 text-sm ",
        xs: "h-7 px-4 has-[>svg]:pl-2 text-xs gap-1 text-xs ",
        xxs: "h-6 px-3 has-[>svg]:pl-2 text-xs gap-1 text-xs ",
        lg: "h-10 px-6 has-[>svg]:pl-5 gap-2.5 text-base ",
        icon: "size-9 ",
