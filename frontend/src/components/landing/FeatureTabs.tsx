...
import {
  Brain, Presentation, Timer, BarChart, Award, Workflow,
  Users, Shield, Clock, Zap, Bot, Layers, Laptop, Camera
} from "lucide-react";
...
export const FeatureTabs = () => {
  const [activeTab, setActiveTab] = useState<'educators' | 'companies' | 'participants'>('educators');

  return (
    <section className="py-16 bg-foreground/[0.02] relative">
      <div className="absolute inset-0 bg-grid-pattern opacity-[0.015] pointer-events-none"></div>
      <div className="container">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold mb-3">Comprehensive Examination Features</h2>
          <p className="text-muted-foreground max-w-[600px] mx-auto">
            Our platform combines question creation, test delivery, monitoring, and analysis in a single solution for schools and companies
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {tabsData[activeTab].map((feature, i) => (
              <MotionDiv 
                ...
                className="group"
              >
                <div className="bg-background border border-border/40 rounded-xl p-6 h-full hover:shadow-md transition-all duration-300 relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-24 h-24 bg-primary/5 rounded-bl-full -z-10"></div>
                  <div className="bg-primary/10 w-12 h-12 rounded-xl flex items-center justify-center mb-4 group-hover:bg-primary/15 transition-colors">
                  ...
                  <div className="flex flex-wrap gap-2 mt-auto">
                    {feature.tags.map((tag, j) => (
                      <div key={j} className="text-xs bg-primary/5 text-primary px-2 py-1 rounded-full">
                        {tag}
                      </div>
                    ))}
                  </div>
          ...
          <div className="bg-background border border-border/40 rounded-xl p-8 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="font-medium text-lg">Comprehensive Proctoring System</h3>
              </div>
              
              <p className="text-muted-foreground mb-6 max-w-[700px]">
                Evaly provides multi-layered security features to ensure exam integrity and prevent cheating for high-stakes examinations
              </p>
              
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { icon: <Camera className="h-4 w-4" />, text: "Webcam Surveillance" },
                  { icon: <Laptop className="h-4 w-4" />, text: "Tab/Window Detection" },
                  { icon: <Bot className="h-4 w-4" />, text: "Voice Detection" },
                  { icon: <Layers className="h-4 w-4" />, text: "Activity Logging" }
                ].map((feature, i) => (
